...
import {
  LAST_THURSDAY_REMINDER_TEXT,
  MONTH_END_REMINDER_TEXT,
  getHourInTokyo,
  isLastDayOfMonthInTokyo,
  isLastThursdayOfMonthInTokyo,
} from "./month-end-reminder.js";
...
function eventTypeToJaLabel(eventType: string): string {
  if (eventType.endsWith(".created")) return "新規作成";
  if (eventType.endsWith(".deleted")) return "削除";
  if (eventType.endsWith(".undeleted")) return "復元";
  if (eventType.endsWith(".moved")) return "移動";
  if (eventType.endsWith(".schema_updated")) return "スキーマ変更";
  if (eventType.endsWith(".properties_updated")) return "プロパティ更新";
  if (eventType.endsWith(".content_updated")) return "内容更新";
  if (eventType.endsWith(".locked")) return "ロック";
  if (eventType.endsWith(".unlocked")) return "ロック解除";
  return "更新";
}
...
function firstTitleFromProperties(
...
function pickEntityPage(body: Record<string, unknown>): NotionPageLike | undefined {
  const candidate = body.entity as NotionPageLike | undefined;
  if (candidate?.properties) return candidate;
  const nested = (body.data as { entity?: NotionPageLike } | undefined)?.entity;
  if (nested?.properties) return nested;
  return undefined;
}
...
async function buildNotionNotificationText(
  ...
  if (title) lines.push(`タスク: ${title}`);
  if (status) lines.push(`ステータス: ${status}`);
  if (priority) lines.push(`優先度: ${priority}`);
  if (due) lines.push(`期日: ${due}`);
  if (assignees.length > 0) lines.push(`担当者: ${assignees.join("、")}`);
...
app.get("/api/diag", (req: Request, res: Response): void => {
  ...
  res.status(200).json({
    ...
    lastVerificationToken:
      "初回検証後は GET /api/notion-verification-token（Bearer CRON_SECRET）で直近の verification_token を取得できます（Vercel ではログが確実）",
    scheduledLineReminders:
      "Vercel Cron: 毎日 12:00 UTC (= 21:00 JST) に GET /api/cron/daily-line（または従来の /api/cron/month-end-reminder）が呼ばれます。東京時間で 21 時台のとき、(1) 月末最終日なら勤怠リマインダー、(2) その月の最終木曜ならミーティング・中掃除リマインダーを送ります。両方該当する日は2通送ります。?dryRun=1 で送信せず状態だけ確認できます。",
  });
});
...
async function handleDailyLineCron(
  ...
  const results: {
    monthEnd: { sent: boolean; error?: string };
    lastThursday: { sent: boolean; error?: string };
  } = {
    monthEnd: { sent: false },
...
app.get(
  ...
  async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
      ...
      return res.status(200).json(JSON.parse(raw) as unknown);
    } catch {
      return res.status(404).json({
        message:
          "まだ verification_token を保存していません。Notion で Webhook を保存して初回 POST を受け取ってください。",
      });
    }
  }
);
...
app.post(
  ...
  async (req: Request, res: Response): Promise<Response> => {
    ...
    if (typeof obj.verification_token === "string" && !("type" in obj)) {
      ...
      console.error(
        "[notion-webhook] verification_token（この値を Notion の入力欄と NOTION_VERIFICATION_TOKEN に貼る）:",
        token
      );
      try {
        await writeFile(
          NOTION_VERIFICATION_STATE_FILE,
          JSON.stringify(
            {
      ...
      return res.status(200).json({
        ...
        verification_token: token,
        message:
          "この verification_token を Notion の入力欄に貼り、検証してください。Vercel の NOTION_VERIFICATION_TOKEN にも同じ値を設定してください。",
      });
    }
